...
# Многопользовательская версия с сохранением состояний и установкой пользовательского времени отправки
...
def id_saver(message):
    ...
    if "error" in check_vk_id_for_validity(vk_id):
        ...
        user_info = {'chat_id': message.chat.id, 'f_name': message.chat.first_name,
                     'l_name': message.chat.last_name, 'vk_id': message.text, 'time_h': 18,
                     'time_min': 0}
        ...
        repl = 'Страница сохранена. Теперь введи удобное время напоминаний в формате чч:мм, например 15:30'
        msg = mybot.send_message(message.chat.id, repl)
        mybot.register_next_step_handler(msg, set_reminder_time)


# Устанавливаем удобное время напоминаний
@mybot.message_handler(commands = ['reset_time'])
def set_reminder_time(message):
    try:
        ...
        if str(message.chat.id) in chat_ids:
            ...
            conv_time = []
            conv_time.append(message.text.split(':'))
            user_info = {'chat_id': message.chat.id, 'f_name': message.chat.first_name,
                        'l_name': message.chat.last_name, 'vk_id': VK_ID, 'time_h': int(conv_time[0][0]),
                        'time_min': int(conv_time[0][1])}
            json.dump(user_info, open('text' + str(message.chat.id) + '.txt', 'w'))
            repl = 'Время установлено. Теперь я буду уведомлять тебя о Днях рождения твоих друзей'
            mybot.send_message(message.chat.id, repl)
        else:
            msg = mybot.send_message(message.chat.id, 'Страница не задана. Для начала воспользуйся /set_id')
    except Exception as ex:
        msg = mybot.send_message(message.chat.id, 'Напиши удобное время')
        mybot.register_next_step_handler(msg, set_reminder_time)
...
# Рассылка напоминаний о Днях рождения
def send_bdays():
    while True:
        chat_ids = paper_work()
        for i in range(len(chat_ids)):
            ...
            user_properties = json.loads(f)
            t_h = user_properties['time_h']
            t_m = user_properties['time_min']
            if (datetime.datetime.now().hour == t_h) & (datetime.datetime.now().minute == t_m):
